...
class DonateHistoryController {
  ...
  static Future<Response> getDonateHistoryByUserHandler(Request request, String userId) async {
    print('[DonateHistoryController] Getting donate history for userId: $userId');
    try {
      ...
      print('[DonateHistoryController] Found ${enrichedHistory.length} sent donations');
      
      return Response.ok(
        jsonEncode(enrichedHistory), 
        headers: {'Content-Type': 'application/json'}
      );
    } catch (e, stackTrace) {
      print('[DonateHistoryController.getDonateHistory] Error: $e \nStack: $stackTrace');
      return Response.internalServerError(body: jsonEncode({'error': e.toString()}));
    }
  }
  ...
  static Future<Response> getReceivedDonateHistoryHandler(Request request, String userId) async {
    print('[DonateHistoryController] Getting received donate history for userId: $userId');
    try {
      // Validate userId format
      if (userId.length != 24 || !RegExp(r'^[a-fA-F0-9]{24}$').hasMatch(userId)) {
        return Response(400, body: jsonEncode({'error': 'Invalid user ID format'}));
      ...
      print('[DonateHistoryController] Found ${enrichedHistory.length} received donations');
      ...
      print('[DonateHistoryController.getReceivedDonateHistory] Error: $e \nStack: $stackTrace');
      return Response.internalServerError(body: jsonEncode({'error': e.toString()}));
    }
  }
}