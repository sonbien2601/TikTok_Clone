...
import 'package:mongo_dart/mongo_dart.dart'
    show ObjectId, where, modify, SelectorBuilder;
...
class ProfileController {
  ...
  static Future<Response> getUserProfileHandler(
    ...
    try {
      ...
      if (e is FormatException && e.message.contains("ObjectId")) {
        return Response(400,
            body: jsonEncode({
              'error': 'Invalid user ID format (could not convert to ObjectId)'
            }));
      }
  ...
  static Future<Response> updateUserProfileHandler(
    ...
    try {
      ...
      if (updateData.containsKey('dateOfBirth')) {
        modifyBuilder =
            modifyBuilder.set('dateOfBirth', updateData['dateOfBirth']);
        hasUpdates = true;
      }
      ...
      if (updateData.containsKey('bankAccountNumber')) {
        modifyBuilder = modifyBuilder.set(
      ...
      if (updateData.containsKey('bankQrImageUrl')) {
        modifyBuilder =
            modifyBuilder.set('bankQrImageUrl', updateData['bankQrImageUrl']);
        hasUpdates = true;
      }
  ...
  static Future<Response> getLikedVideosHandler(
    ...
    try {
      ...
      final likedVideosCursor = videosCollection.find(SelectorBuilder()
      ...
      final totalLikedVideos =
          await videosCollection.count(where.eq('likes', userObjectId));
  ...
  static Future<Response> getSavedVideosHandler(
    ...
    try {
      ...
      final savedVideoIds =
          (userDoc['savedVideos'] as List?)?.whereType<ObjectId>().toList() ??
              [];
      ...
      final savedVideosCursor = videosCollection.find(SelectorBuilder()
  ...
  static Future<Response> getUserVideosHandler(
    ...
    try {
      ...
      // Get user's videos
      final userVideosCursor = videosCollection.find(SelectorBuilder()
      ...
      final totalUserVideos =
          await videosCollection.count(where.eq('userId', userObjectId));
  ...
  static Map<String, dynamic> _formatVideoDocument(
    ...
    if (formattedVideo['userId'] is ObjectId) {
      formattedVideo['userId'] =
          (formattedVideo['userId'] as ObjectId).toHexString();
    }
