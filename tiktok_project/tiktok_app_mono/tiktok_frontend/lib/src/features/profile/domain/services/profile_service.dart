...
class ProfileService {
  ...
  // Method này được EditProfilePage gọi
  ...
  Future<Map<String, dynamic>> getLikedVideos(String userId, {int page = 1, int limit = 10}) async {
    try {
      final baseUrl = await NetworkConfig.getBaseUrl('/api/users');
  ...
  Future<Map<String, dynamic>> getSavedVideos(String userId, {int page = 1, int limit = 10}) async {
    try {
      final baseUrl = await NetworkConfig.getBaseUrl('/api/users');
  ...
  Future<Map<String, dynamic>> getUserVideos(String userId, {int page = 1, int limit = 10}) async {
    try {
      final baseUrl = await NetworkConfig.getBaseUrl('/api/users');
